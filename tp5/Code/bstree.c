...
void node_to_dot(const BinarySearchTree *t, void *userData) {
    ...
    if (bstree_left(t)) {
        ...
        fprintf(file, "\tlnil%d [style=filled, fillcolor=red, label=\"NIL\"];\n", bstree_root(t));
    ...
    if (bstree_right(t)) {
        ...
        fprintf(file, "\trnil%d [style=filled, fillcolor=red, label=\"NIL\"];\n", bstree_root(t));
...
void rbtree_export_dot(const BinarySearchTree *t, FILE * file ) {
    fprintf (file , " digraph RedBlackTree {\n\ tgraph[ ranksep =0.5];\ n\ tnode [ shape = record ];\n\n");
...
ptrBinarySearchTree fixredblack_insert_case1(ptrBinarySearchTree x){
    ...
    if ((f != NULL) && (f->color == red)) {
        ...
        f->color = black;
        pp->color = red;
        return fixredblackproperties_insert(pp);
    } else {
        return fix_insert_case2(x);
    }
}
...
ptrBinarySearchTree fixredblack_insert_case2(ptrBinarySearchTree x){
    ...
    ptrBinarySearchTree pp = grandparent(x);
    if (pp->left == p)
        x = fix_insert_case2_left(x);
    else
        x = fix_insert_case2_right(x);
    return x;
}
...
ptrBinarySearchTree fix_insert_case2_right(ptrBinarySearchTree x) {
    ptrBinarySearchTree p = x->parent;
    ptrBinarySearchTree pp = grandparent(x);
    if (x == p->right) {
        leftrotate(pp);
...
void bstree_remove_node (ptrBinarySearchTree *t, ptrBinarySearchTree current){
    ...
    if (current->color == black) {
        ...
        ptrBinarySearchTree subtreeroot = fixredblackproperties_remove(current->parent, substitute);
...
ptrBinarySearchTree fixredblack_remove(ptrBinarySearchTree p, ptrBinarySearchTree x){
    ...
    if ( b->color==black )
        return fixredblackproperties_remove_case1(p, x);
    else
        return fixredblackproperties_remove_case2(p, x);
    }
}
...
ptrBinarySearchTree fixredblack_remove_case1_left(ptrBinarySearchTree p, ptrBinarySearchTree x){
    ...
    if ( ( (b->left == NULL) || (b->left->color == black) ) &&
        ...
        if ( p->color == red) {
            ...
            return fixredblackproperties_remove(p->parent, p);
        }
...
ptrBinarySearchTree fixredblack_remove_case1_right(ptrBinarySearchTree p, ptrBinarySearchTree x) {
    ...
    if (((b->right == NULL) || (b->right->color == black)) &&
        ((b->left == NULL) || (b->left->color == black))) {
        /* case 1.a : both sons are black or null leaf */
        b->color = red;
        if (p->color == red) {
            p->color = black;
...
ptrBinarySearchTree fixredblack_remove_case1(ptrBinarySearchTree p, ptrBinarySearchTree x){
    ...
    if (x == p->left)
        return fixredblackproperties_remove_case1_left(p, x);
    else
        return fixredblackproperties_remove_case1_right(p, x);
}
...
ptrBinarySearchTree fixredblack_remove_case2_left(ptrBinarySearchTree p, ptrBinarySearchTree x){
    ...
    p->color = red;
    f->color = black;
    fixredblackproperties_remove_case1_left(p, x);
    return f;
}
...
ptrBinarySearchTree fixredblack_remove_case2_right(ptrBinarySearchTree p, ptrBinarySearchTree x) {
    ...
    p->color = red;
    f->color = black;
    fixredblackproperties_remove_case1_right(p, x);
    return f;
}
...
ptrBinarySearchTree fixredblack_remove_case2(ptrBinarySearchTree p, ptrBinarySearchTree x){
    /* x is not the root of the tree */
    if (x == p->left)
        return fixredblackproperties_remove_case2_left(p, x);
    else
        return fixredblackproperties_remove_case2_right(p, x);
}
